...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/alarm/alarm/">报警记录列表</a></li>
		<li class="active"><a href="${ctx}/alarm/alarm/form?id=${alarm.id}">报警记录<shiro:hasPermission name="alarm:alarm:edit">${not empty alarm.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="alarm:alarm:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="alarm" action="${ctx}/alarm/alarm/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="hostNumber" htmlEscape="false" maxlength="100" class="input-xlarge "/>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">回路号：</label>
			<div class="controls">
				<form:input path="loopNumber" htmlEscape="false" maxlength="100" class="input-xlarge "/>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="form-actions">
			<shiro:hasPermission name="alarm:alarm:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
